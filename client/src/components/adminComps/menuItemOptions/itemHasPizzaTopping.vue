  ...
  <div class="row justify-center">
    <q-form ref="myForm" v-if="updateMenuItemObj">
      ...
      <div
        ...
        v-if="updateMenuItemObj.hasPizzaToppings"
      >
        <q-select
          outlined
          v-model="updateMenuItemObj.chosenPizzaToppings"
          :options="pizzaToppingOptions"
...
export default {
  ...
  computed: {},
  watch: {
    "updateMenuItemObj.hasPizzaToppings"() {
      if (!this.updateMenuItemObj.hasPizzaToppings) {
        this.updateMenuItemObj.chosenPizzaToppings = [];
      }
    }
  },
  beforeCreate() {},
  ...
  methods: {
    resetFields() {
      this.updateMenuItemObj.chosenPizzaToppings = [];
      this.updateMenuItemObj.hasPizzaToppings = false;
    ...
    filterFnToppings(val, update) {
      if (val === "") {
        update(() => {
          this.pizzaToppingOptions = _.sortBy(
            this.$store.getters.getAdminSideItems.filter(item =>
              item.addonCategory.name.includes("pizza-toppings")
            ),
            function(user) {
